...
process coolerSort {
   ...
   if [ -e /cooler ]; then
    ...
    cooler csort --sort-options '-S 2G --parallel=${task.cpus}' -i tabix -c1 2  -p1 3 -s1 4 -c2 9 -p2 10 -s2 11 -p ${task.cpus} -o hic_${sample}.scf.bgz ${chrfile} <(zcat -f ${reads} | awk '{if (\$8 >= ${options['mapq']} && \$15 >= ${options['mapq']}) print;}')
   """
}
